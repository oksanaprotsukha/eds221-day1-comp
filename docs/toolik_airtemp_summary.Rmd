...
toolik_weather <- read_csv(here::here("data", "data-raw", "toolik_weather.csv"), na = "#N/A") %>% 
  # transform column names in the table to a snake case
  janitor::clean_names()
```
...
```{r}
 jitterChartExample1 <- ggplot(data = toolik_weather, aes(x = month, y = daily_air_temp_mean_c))+
  ...
  ggsave(here::here("figs","jitterChartExample1.png"),plot = jitterChartExample1, height = 5, width = 6 )
